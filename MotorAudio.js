...
export class MotorAudio {
    ...
    playPezSplash(tiempoActual) {
        ...
        const noise = this.crearFuenteRuido();
        const filtro = this.audioCtx.createBiquadFilter();
        filtro.type = 'lowpass'; filtro.frequency.setValueAtTime(800, tiempoActual);
    ...
    playPop() {
        // EL SEGURO DE VIDA: Si el contexto está suspendido, no encolamos el sonido.
        if (!this.sfxEnabled || !this.audioCtx || !this.masterGainSFX || this.audioCtx.state === 'suspended') return;
        try {
    ...
    toggleMusic() {
        ...
        this.initCtx(); 
        if (this.musicEnabled) this.startZenMusic();
        else this.stopZenMusic();
    ...
    initZenMusicEngine() {
        ...
        this.musicMaster = this.audioCtx.createGain(); this.musicMaster.gain.value = 0.0;
        this.musicDry = this.audioCtx.createGain(); this.musicDry.gain.value = 0.75;
        this.musicReverbSend = this.audioCtx.createGain(); this.musicReverbSend.gain.value = 0.35;
        this.musicCompressor = this.audioCtx.createDynamicsCompressor();
        this.musicCompressor.threshold.value = -26; this.musicCompressor.knee.value = 18;
        this.musicCompressor.ratio.value = 2.2; this.musicCompressor.attack.value = 0.02; this.musicCompressor.release.value = 0.45;
    ...
    _createImpulseResponse(seconds, decay) {
        const rate = this.audioCtx.sampleRate; const length = Math.floor(rate * seconds);
    ...
    _scheduleNote({ time, freq, duration = 6, peak = 0.35, type = 'sine', brightness = 0.24, vibratoAmount = 2.5, vibratoRate = 0.12, reverbSendBoost = 1.0 }) {
        ...
        try {
            const osc = this.audioCtx.createOscillator(); const amp = this.audioCtx.createGain(); const filter = this.audioCtx.createBiquadFilter();
            osc.type = type; filter.type = 'lowpass'; filter.frequency.value = 700 + brightness * 2200; filter.Q.value = 0.35; amp.gain.value = 0;
            osc.connect(filter).connect(amp); amp.connect(this.musicDry); amp.connect(this.musicReverbSend);
            const lfo = this.audioCtx.createOscillator(); const lfoGain = this.audioCtx.createGain();
            lfo.type = 'sine'; lfo.frequency.value = vibratoRate; lfoGain.gain.value = vibratoAmount; lfo.connect(lfoGain).connect(osc.frequency);
            osc.frequency.setValueAtTime(freq, time);
            amp.gain.setValueAtTime(0.0001, time); amp.gain.linearRampToValueAtTime(peak, time + Math.min(2.2, duration * 0.35)); amp.gain.exponentialRampToValueAtTime(0.0001, time + duration);
            const baseCutoff = 700 + brightness * 2200; filter.frequency.setValueAtTime(baseCutoff, time); filter.frequency.linearRampToValueAtTime(baseCutoff * 1.08, time + duration * 0.2); filter.frequency.linearRampToValueAtTime(baseCutoff * 0.92, time + duration);
            const originalSend = this.musicReverbSend.gain.value; this.musicReverbSend.gain.setValueAtTime(originalSend * reverbSendBoost, time);
            osc.start(time); lfo.start(time); osc.stop(time + duration + 0.1); lfo.stop(time + duration + 0.1);
        } catch(e) {}
    }

    _scheduleGhostVoice({ time, baseFreq, duration }) {
        const intervalChoices = [1, 3 / 2, 2]; const ratio = intervalChoices[Math.floor(Math.random() * intervalChoices.length)]; const ghostFreq = baseFreq * ratio;
        this._scheduleNote({ time: time + 0.8 + Math.random() * 1.2, freq: ghostFreq, duration: duration * (0.7 + Math.random() * 0.5), peak: 0.12, type: 'triangle', brightness: this.sessionMood.brightness * 0.9, vibratoAmount: 1.2, vibratoRate: 0.08, reverbSendBoost: 1.25 });
    }
    ...
    _schedulePhrase(startTime) {
        ...
        let t = startTime; let firstFreq = null; let lastFreq = null;
        for (let i = 0; i < noteCount; i++) {
            this.currentDegree = this._weightedNextDegree(); this.currentRegister = this._maybeShiftRegister();
            if (Math.random() < 0.20) { this.currentDegree = 0; if (Math.random() < 0.8) this.currentRegister = 1; }
            const freq = this._degreeToFreq(this.currentDegree, this.currentRegister); const duration = 4.8 + Math.random() * 4.5; const peak = 0.25 + Math.random() * 0.15; const type = Math.random() < 0.85 ? 'sine' : 'triangle';
            ...
            if (firstFreq == null) firstFreq = freq; lastFreq = freq; t += 2.2 + Math.random() * 3.8;
        }
        if (Math.random() < this.sessionMood.ghostVoiceChance && firstFreq) this._scheduleGhostVoice({ time: startTime, baseFreq: Math.random() < 0.5 ? firstFreq : lastFreq, duration: 5 + Math.random() * 3 });
        const phraseDuration = t - startTime; const silence = (6 + Math.random() * 10) + (noteCount > 1 ? (noteCount - 1) * (2 + Math.random() * 2) : 0);
    ...
    _zenLoop = () => {
        ...
        try {
            const now = this.audioCtx.currentTime; const start = Math.max(now + 0.05, this.lastPhraseEndedAt || now + 0.05);
            const drift = (v, amt, min, max) => Math.max(min, Math.min(max, v + (Math.random() * 2 - 1) * amt));
            this.sessionMood.activity = drift(this.sessionMood.activity, 0.035, 0.18, 0.45); this.sessionMood.brightness = drift(this.sessionMood.brightness, 0.03, 0.15, 0.38); this.sessionMood.ghostVoiceChance = drift(this.sessionMood.ghostVoiceChance, 0.03, 0.08, 0.28);
        ...
        } catch (e) { this.zenTimeoutId = setTimeout(this._zenLoop, 5000); }
    }
